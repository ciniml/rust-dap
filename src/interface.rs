...
fn write_descriptor_set(buffer: &mut [u8], windows_version: u32, f: impl FnOnce(&mut [u8]) -> Result<usize>) -> Result<usize> {
    let length = 10usize;
...
fn write_configuration_subset(buffer: &mut [u8], f: impl FnOnce(&mut [u8]) -> Result<usize>) -> Result<usize> {
    let length = 8usize;
    if buffer.len() < length {
        return Err(UsbError::BufferOverflow);
    }
...
fn write_function_subset(buffer: &mut [u8], first_interface_number: InterfaceNumber, f: impl FnOnce(&mut [u8]) -> Result<usize>) -> Result<usize> {
    let length = 8usize;
    if buffer.len() < length {
        return Err(UsbError::BufferOverflow);
    }
...
fn write_compatible_id(buffer: &mut [u8], compatible_id: &[u8; 8], sub_compatible_id: &[u8; 8]) -> Result<usize> {
    let length = 20usize;
    if buffer.len() < length {
        return Err(UsbError::BufferOverflow);
    }
...
fn write_registry_property(buffer: &mut [u8], property_name: &[u8], property_type: RegPropertyType, property_data: &[u8]) -> Result<usize> {
...
const fn u16_lo(v: u16) -> u8 { (v & 0xff) as u8 }
const fn u16_hi(v: u16) -> u8 { (v >> 8) as u8 }
const fn u32_lo(v: u32) -> u16 { (v & 0xfffff) as u16 }
const fn u32_hi(v: u32) -> u16 { (v >> 16) as u16 }
...
impl<B: UsbBus> UsbClass<B> for CmsisDapInterface <'_, B> {
    ...
    fn control_in(&mut self, xfer: ControlIn<B>) {
        ...
        if request.request_type == RequestType::Vendor && request.request == MS_VENDOR_CODE && request.value == 0 && request.index == 7 {
                ...
                write_descriptor_set(buffer, 0x06030000, |buffer| {
                    write_configuration_subset(buffer, |buffer| {
                        write_function_subset(buffer, interface_number, |buffer| {
                            ...
                            let mut property_data = [0u8; b"{A5DCBF10-6530-11D2-901F-00C04FB951ED}\0".len()*2];
                            fill_utf16(&mut property_data, b"{A5DCBF10-6530-11D2-901F-00C04FB951ED}\0");
                            
                            // Set Compatible ID to WINUSB in order to be WinUSB driver is loaded for this interface. 
                            offset += write_compatible_id(&mut buffer[offset..], b"WINUSB\0\0", &[0u8; 8])?;
                            // Set GUID_DEVINTERFACE_USB_DEVICE({A5DCBF10-6530-11D2-901F-00C04FB951ED}) Device Interface Class to be enumerated by libusb.
                            offset += write_registry_property(&mut buffer[offset..], &property_name, RegPropertyType::String, &property_data)?;
                            Ok(offset)
                        })
                    })
                })
