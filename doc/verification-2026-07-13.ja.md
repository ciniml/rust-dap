...
## 追記21: M-nRF1(nRF52832 接続実証)実機検証 (2026-07-16)

ユーザのカスタム nRF52832 ボードをプローブに接続(GPIO2=SWCLK, GPIO3=SWDIO,
GND 共通、nRF52 側は別電源)。nrf52_probe ファームの出力:
...
| # | テスト | 結果 |
|---|---|---|
| 1 | `connect_swd`(line reset + JTAG→SWD 0xE79E)で SW-DP 接続 | OK |
| 2 | DPIDR = 0x2ba01477(nRF52 SW-DP IDCODE) | 一致 |
| 3 | CTRL-AP(APSEL 1)IDR = 0x02880000 到達(APSEL 対応) | OK |
| 4 | APPROTECTSTATUS = OPEN(保護なし) | OK(施錠されておらず erase_all 不要) |
| 5 | AHB-AP 経由 FICR.INFO.PART = 0x52832 読み出し | 一致 |

**M-nRF1 完了**。connect_swd / APSEL / CTRL-AP / APPROTECT 判定 / FICR 読みが
...
## 追記26: nRF54 対応 + RTT 高速化(ビルド通過)+ auto 検出の nRF52 回帰 (2026-07-16)

nRF54 実機は未入手のため、arm-debug 層 + gdb_server の family を「ビルド通過」まで
実装(branch: nrf54-support)。RTT アップストリームのチャンク拡大も実施。

**nRF54(build-pass)**:
- arm-debug: FPB rev2(FP_CTRL.REV でランタイム分岐、v2 は BPADDR[31:1]|BE で
  全アドレス空間対応)、nrf54 モジュール(DPv2 DPIDR 0x6BA02477、CTRL-AP #2、
  RRAMC = 消去不要・commit 方式のフラッシュ)。ホストテスト 28 件(FPBv2 符号化・
  RRAM commit 追加)。
- gdb_server: `Nrf54Family`(`gdb-target-nrf54`)= connect_swd・単一 M33 コア・
  RRAM 書込(消去なし・端数は read-modify-write)・1.5MB RRAM / 240KB SRAM。
  auto 検出は 1 回の SW-DP bring-up で DPIDR により nRF52/nRF54 を振り分け。
- 全 family 構成(rp2040 / nrf52 / nrf54 / auto 全部盛り)で警告ゼロ。
- 既知の未対応: ARMv8-M の DWT は MASK レジスタ廃止のため、現行 watchpoint_set は
  nRF54 では要改修(実機入手時に対応)。

**RTT 高速化**: アップストリームを 1 ポーリング 64B → 256B のブロック読みに拡大
(記述子読み + RdOff 書戻しの固定ラウンドトリップをより多くのペイロードで償却)。

**auto 検出の nRF52 実機回帰**(nRF54 追加が既存を壊していないことの確認):
auto 全部盛りビルドを書き込み、nRF52832 ターゲットで attach:

| # | テスト | 結果 |
|---|---|---|
| 1 | auto ビルドが nRF52 を自動検出(単一スレッド、part 0x52832、approtect OPEN) | OK |
| 2 | RTT scan(短命 RTT のため no control block = 既知挙動) | OK(誤検出なし) |

RTT スループットの実測は、RP2040 blink_demo(出力律速)/短命 nRF RTT のいずれも
測定に不向きなため、連続出力の RTT ソースが用意でき次第実施。

**注記**: xiao_m0 RTIC 化は別 branch(xiao-m0-rtic)、nRF54+RTT は branch
(nrf54-support)。いずれも main 未マージ(実機検証・レビュー後に PR 化予定)。

## 追記27: usb-device 0.3 / embedded-hal 1.0 / rp2040-hal 0.10 一括 bump (2026-07-16)

ワークスペース協調 bump(branch: dep-bump-usb-device-0.3):usb-device 0.2→0.3、
usbd-serial 0.1→0.2、embedded-hal 0.2→1.0、rp2040-hal 0.8→0.10、rp-pico 0.7→0.9、
xiao_m0 BSP 0.11→0.13(atsamd-hal 0.17)。RTIC は 1.0 のまま(2.0 は不要・利点なし)。

主な API 変更:
- usb-device: `get_string(_, LangID)`、builder の strings API(StringDescriptors)、
  `max_packet_size_0()?`、usbd-serial `ParityType::Event`→`Even`。
- embedded-hal 1.0: `digital::v2`→`digital`、ToggleableOutputPin→StatefulOutputPin、
  UART は embedded-hal-nb。PicoBidirPin/XiaoBidirPin を 1.0 トレイトへ。
- rp2040-hal 0.10: gpio 3 引数 `Pin<Id, Function, Pull>`、`into_mode`→`into_function`、
  PIO ピン番号は `.id().num`(型 const 廃止)、`sie_ctrl`→`sie_ctrl()`。
- xiao_m0: gpio v2 モジュール平坦化、cortex-m `critical-section-single-core` 追加。

| # | テスト | 結果 |
|---|---|---|
| 1 | 全ワークスペースビルド(rpi_pico 全 feature / xiao_rp2040 / xiao_m0) | 0 エラー |
| 2 | host tests(rust-dap 12 / arm-debug 26) | 全パス |
| 3 | usb-device 0.3 firmware の実機列挙(2 CDC、strings 正常) | OK |
| 4 | 1200bps タッチ書込 + attach + auto 検出(nRF52、DPIDR 0x2ba01477、connect OK) | OK |

usb-device 0.3 スタック + bitbang SWD(e-hal 1.0 ピン)+ RSP + arm-debug が
実機で機能することを確認。**実機回帰の結論(回帰ではなかった)**: RP2040 ターゲットを接続して回帰実行した際、
usb-device 0.3 firmware は列挙・生 RSP(qSupported 完全応答)・初回 attach
(2 スレッド、DPIDR 0x0bc12477、connect OK)まで動いたが、その後の attach で
SWD connect が失敗(diag[2]=0x107 = ACK 0b111 = 応答なし、DPIDR=0)。当初は
移行回帰を疑ったが、**移行前 firmware(nrf54-support ブランチ、以前確実に動作)
を同一ターゲットに焼いても全く同じ症状で失敗**(`remote failure reply '79'`、
No threads)。同じ移行 firmware で nRF52 は安定動作。よって**firmware の回帰
ではなく RP2040 ターゲット側の問題**(SWD が応答しない状態 — 配線・電源・
ターゲット実行中プログラムによる SWD 無効化・要電源再投入)と結論。
SWD clock を遅く(clock_wait=20)しても改善せず、タイミングでもない。
→ dep-bump 移行は列挙・CDC・RSP・初回 connect まで実機確認済み。RP2040 の
フル回帰(E2E/reattach/dual-core/load/RTT/reset)はターゲット復旧後に再実施。

**追加調査(ターゲット復旧試行)**: probe 物理抜き差し + ターゲット BOOTSEL
リセット後、クリーン初回接続は毎回成功(2 スレッド、DPIDR 0x0bc12477、
RAM 0x20000000 読み OK)を再現。移行の機能は確定。ただし:
- ターゲットが**空フラッシュ(bootrom-USB 実行中)**のため、1 回の
  connect+detach 後に必ず wedge(次接続で '79' = AP ACK エラー)。
  → 通常プログラム(blink)を焼けば安定するはずだが、load はリセット直後の
  "最初の接続" でしか実行できず、確認接続を挟むと wedge して load 不可。
- 繰り返しセッション下で firmware 側の堅牢性問題が顕在化:
  (a) connect_and_halt が特定ターゲット状態で長時間ハング、
  (b) QueueConn::write() の tx 満杯時 unbounded loop(ホスト無読取時ハング要因)、
  (c) RSP 応答にゴミ混入(gdb: `unrecognized item "timeout" in qSupported`)=
      CDC バイト破損/キュー競合の疑い。初回は正常なので移行が原因ではなく
      繰り返しストレスで顕在化。
これらは**移行(dep-bump)とは独立の既存堅牢性課題**(移行前 firmware も
