...
実機で機能することを確認。**実機回帰で判明した回帰(要修正)**: RP2040 ターゲットを接続して回帰実行したところ、
usb-device 0.3 firmware は列挙・生 RSP 応答・初回 attach(2 スレッド、DPIDR
0x0bc12477、connect OK)まで一度は動いたが、**RP2040 の SWD multidrop 接続が
不安定**(以後の attach で connect_and_halt が失敗しメモリ/レジスタ読み不可)。
同じ firmware で nRF52(connect_swd 経由)は安定動作したため、bitbang SWD の
一般的なピン駆動ではなく、**RP2040 固有の multidrop(dormant/TARGETSEL の生ビット
シーケンス)経路のタイミング回帰**の疑い。rp2040-hal 0.10 の gpio 操作
(into_function/set_state/is_high)のレイテンシ変化が bitbang SWD タイミングに
影響した可能性。**マージ前に要調査・修正**(PIO SWD 経路の実機確認も未了)。
