...
- **制御ブロックは 1 つとは限らない(要件)**: nRF Connect SDK のように
  bootloader(MCUboot)とアプリケーションが**別アドレスにそれぞれ
  `_SEGGER_RTT` を持つ**構成がある。ブート後も bootloader 側のマジックが
  RAM に残留し得るため、スキャンは最初の 1 個で打ち切らず**全候補を列挙**し、
  どれを使うか選択できるようにする:
  - `monitor rtt scan` — 全候補を列挙(アドレス、up/down 本数、各チャネルの
    名前文字列、バッファアドレス/サイズ、valid/stale 判定)。stale 判定の
    ヒント: WrOff/RdOff がバッファサイズ範囲外なら壊れている、実行中に
    WrOff が進む方が生きている。
  - `monitor rtt attach <addr|index>` — 使用する制御ブロックを明示選択。
    (`setup` はスキャン省略の直接指定、`attach` はスキャン結果からの選択。)
...
```rust
struct RttSession {
    /// 選択中の制御ブロック先頭(scan/attach/setup で確定)
    cb: Option<u32>,
    /// 選択中のチャネル(up/down のバッファ index、既定 0 = "Terminal")
    channel: u8,
    /// 選択チャネルのバッファ記述子アドレス(cb+24+24*ch, up 側は
    /// cb+24+24*(max_up+ch))とキャッシュした pBuffer/SizeOfBuffer
    up_desc: u32, down_desc: u32,
    up_buf: u32, up_size: u32, down_buf: u32, down_size: u32,
}
```

チャネルについて: 1 つの制御ブロック内に up(target→host)/down(host→target)
それぞれ**バッファ配列**があり、その index が「チャネル」。慣例は
ch0 = "Terminal"(ログ、defmt-rtt も ch0)、ch1 = SystemView 等。既定は ch0 と
し、`monitor rtt channel <n>` で切替可能にする(第 2 CDC は 1 本なので同時に
橋渡しするのは 1 チャネル。複数チャネル同時が欲しくなったら CDC を増やす)。
なお SEGGER の「Virtual Terminal」(ch0 内の 0xFF エスケープによる 16 端末
多重化)はチャネルとは別物で、ホスト側表示の問題なのでプローブは素通しする。
...
```text
monitor rtt scan           — RAM を走査し全制御ブロックを列挙(valid/stale 付き)
monitor rtt attach <addr>  — 使用する制御ブロックを選択(scan 結果から)
monitor rtt setup <addr>   — CB アドレス直接指定(スキャン省略の attach)
monitor rtt channel <n>    — 橋渡しするチャネル(バッファ index)切替、既定 0
monitor rtt status         — 選択中 CB/チャネル/転送量の表示
